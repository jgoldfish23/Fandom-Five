      ...
      <div className="rounded-3xl p-6 mb-3 text-center relative overflow-hidden card-hover" style={accentBg}>
        {LOGOS[p.key] ? (
          <div className="mx-auto mb-2 flex items-center justify-center" style={{ width: 92, height: 92, borderRadius: 24, background: "rgba(255,255,255,0.94)", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 8px 22px rgba(0,0,0,0.22)" }}>
            <img src={LOGOS[p.key]} alt={p.name + " logo"} style={{ maxWidth: 72, maxHeight: 72, objectFit: "contain" }} />
          </div>
...
const cardEmoji = name => (HOME_CARDS.find(c => c.name === name || name.includes(c.name.split(" ").pop())) || {}).emoji || "🏟";
      ...
      <div className="rounded-3xl p-3" style={glassH}>
        ...
        {(games || []).map((g, i) => (
          <div key={i} className="flex items-center gap-2.5 py-1.5 text-sm border-b last:border-0" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
            <span className="text-lg shrink-0">{cardEmoji(String(g.team || ""))}</span>
        ...
        {(news || []).map((n, i) => {
          const inner = (
            <div className="flex items-start gap-2.5 py-2 border-b last:border-0" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
              <span className="text-base shrink-0">{cardEmoji(String(n.team || ""))}</span>
  ...
  const Side = ({ side }) => (
    <div className="flex gap-1 flex-wrap">
      {keys.map(k => { const c = HOME_CARDS.find(x => x.key === k); const on = pair[side] === k; return (
        <button key={k} onClick={() => pickSide(side, k)} title={c.name} className="btn-lift w-9 h-9 rounded-xl text-lg" style={on ? { background: "#fff", boxShadow: "0 0 14px rgba(150,175,255,0.6)" } : { background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.16)" }}>{c.emoji}</button>
      ); })}
    </div>
  );
      ...
      <div className="rounded-3xl p-3 mb-6" style={glassH}>
        ...
        {(scores || []).map((x, i) => (
          <div key={i} className="flex items-center gap-2 py-1.5 text-sm border-b last:border-0" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
            <span className="text-base shrink-0">{cardEmoji(String(x.team || ""))}</span>
...
function PollBoard({ kind, ui }) {
  ...
  const load = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-5", max_tokens: 3000, tools: [{ type: "web_search_20250305", name: "web_search" }], messages: [{ role: "user", content: `Search the web for ${cfg.ask}. Respond with ONLY a JSON array (no prose, no markdown) of {"rk": number, "team": string, "rec": string} for all 25 teams in order.` }] }),
      });
      const data = await res.json();
      const text = (data.content || []).filter(b => b.type === "text").map(b => b.text).join("\n");
      let items = [];
    ...
    <div className={"min-h-screen w-full relative overflow-hidden theme-" + theme} style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-12 relative" style={{ zIndex: 1 }}>
        <div className="pt-8 pb-4 text-center flex flex-col items-center relative">
          ...
          {LOGOS.byufootball ? (
            <img src={LOGOS.byufootball} alt="BYU stretch Y" style={{ width: 104, marginBottom: 8, filter: "drop-shadow(0 6px 14px rgba(42,79,224,0.4))" }} />
